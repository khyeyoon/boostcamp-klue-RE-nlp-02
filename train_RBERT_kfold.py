...
from load_data_entity_ex import *
...
def train(args):
    ...
    for kfold_idx, (train_idx, valid_idx) in enumerate(skf.split(dataset, dataset_label)):
        ...
        model=RE_NET_v2(backbone=model, tokenizer=tokenizer, class_num=model_config.num_labels, hidden_size=h, drop_rate=args.dropout,device=device)
        ##############################모델 불러오기############################
        # model = AutoModelForSequenceClassification.from_pretrained('./HY_roberta_sampler/best_loss')
        model.to(device)
        ...
        # tokenizing dataset
        tokenized_train,pos_index_train = tokenized_dataset(train_dataset, tokenizer,sep_type=args.sep_type)
        tokenized_valid,pos_index_valid = tokenized_dataset(valid_dataset, tokenizer,sep_type=args.sep_type)

        # make dataset for pytorch.
        RE_train_dataset = RE_Dataset(tokenized_train,train_label,pos_index_train)
        RE_valid_dataset = RE_Dataset(tokenized_valid,valid_label,pos_index_valid)
